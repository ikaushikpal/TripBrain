...
public class DatabaseViewManager {
    ...
    public void initView() {
        try {
            // Drop legacy objects with this name first to ensure it is created as a
            // materialized view
            try {
            ...
            jdbcTemplate
                    .execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_gallery_view_id ON public_trip_gallery_view(id);");
            log.info("PostgreSQL Materialized View public_trip_gallery_view initialized successfully.");
        } catch (Exception e) {
            log.warn("Failed to initialize materialized view. Database might not be PostgreSQL yet: {}",
                    e.getMessage());
        }
    }
