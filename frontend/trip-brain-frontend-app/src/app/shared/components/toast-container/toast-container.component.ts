    ...
    <div class="fixed bottom-6 right-6 z-[9999] flex flex-col gap-3 max-w-sm w-full pointer-events-none">
      @for (toast of notificationService.toasts(); track toast.id) {
        ...
        >
          ...
          <!-- Info Icon -->
          @if (toast.type === 'info') {
            <svg class="w-5 h-5 shrink-0 text-blue-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button class="text-xs opacity-50 hover:opacity-100 transition-opacity shrink-0 ml-auto select-none">
